...
2. **Auto-create a linked, standalone Zoho task** for it — every issue gets one, no case-by-case judgment call:
   ...
   ```
   Create the Zoho task first (`create_a_task`, in the current task list or a small maintenance list if none fits), reference the issue number in its description, then record it with the command above.
